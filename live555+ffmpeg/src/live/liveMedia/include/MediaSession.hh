...
public:
	...
	{
		fReceiveRawMP3ADUs = True;    // optional hack for audio/MPA-ROBUST; must not be called after initiate()
	}
	void receiveRawJPEGFrames()
	{
		fReceiveRawJPEGFrames = True;    // optional hack for video/JPEG; must not be called after initiate()
	}
	...
	{
		return attrVal_str("sprop-parameter-sets");
	}
	char const *fmtp_spropvps() const
	{
		return attrVal_str("sprop-vps");
	}
	char const *fmtp_spropsps() const
	{
		return attrVal_str("sprop-sps");
	}
	char const *fmtp_sproppps() const
	{
		return attrVal_str("sprop-pps");
	}
